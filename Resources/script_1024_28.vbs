Option Explicit
' -----------------------------------------------------------------------------
' Fan Sensor Information - Alert When Not OK
' -----------------------------------------------------------------------------
' Parameters:
...
' -----------------------------------------------------------------------------
	...
	Public Function getFanCheckInformation
		Dim Caption, HealthState, xmlDom
		Dim strBase : strBase = getBaseURL("OMC_Fan")
		Dim strQueryResource : strQueryResource = strBase & "OMC_Fan"
		...
		While Not oQueryResponse.AtEndOfStream
			...
			Caption = ParseXML(xmlDom, "/n1:OMC_Fan/n1:Caption")
			HealthState = Replace(ParseXML(xmlDom, "/n1:OMC_Fan/n1:HealthState"), "NULL", "0")
			
			updateSummaryHealthCounts HealthState
			
			sXmlDocContent = sXmlDocContent & "<sensor type='OMC_Fan' caption='" & Caption & "' health_state='" & HealthState & "'/>"
...
' get fan information
sXmlDocContent = sXmlDocContent & "<fancheck>"
getFanCheckInformation
sXmlDocContent = sXmlDocContent & "</fancheck>"
