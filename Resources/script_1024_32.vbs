Option Explicit
' -----------------------------------------------------------------------------
' PSU Sensor Information - Alert When Not OK
' -----------------------------------------------------------------------------
' Parameters:
...
' -----------------------------------------------------------------------------
	...
	Public Function getPsuCheckInformation
		Dim Caption
		Dim HealthState
		Dim EnabledDefault
		Dim EnabledState
		Dim TotalOutputPower
		Dim Range1InputFrequencyHigh
		Dim Range1InputFrequencyLow
		Dim Range1InputVoltageHigh
		Dim Range1InputVoltageLow
		Dim xmlDom
		
		Dim strBase : strBase = getBaseURL("OMC_PowerSupply")
		Dim strQueryResource : strQueryResource = strBase & "OMC_PowerSupply"
		...
		While Not oQueryResponse.AtEndOfStream
			...
			Caption = ParseXML(xmlDom, "/n1:OMC_PowerSupply/n1:Caption")
			TotalOutputPower = ParseXML(xmlDom, "/n1:OMC_PowerSupply/n1:TotalOutputPower") 
			TotalOutputPower = Replace(ParseNumber( TotalOutputPower ) / 1000 & " W", "0 W", "-")
			HealthState = Replace(ParseXML(xmlDom, "/n1:OMC_PowerSupply/n1:HealthState"), "NULL", "0")
			EnabledDefault = ParseXML(xmlDom, "/n1:OMC_PowerSupply/n1:EnabledDefault")
			EnabledState = ParseXML(xmlDom, "/n1:OMC_PowerSupply/n1:EnabledState")
			Range1InputFrequencyHigh = Replace(ParseXML(xmlDom, "/n1:OMC_PowerSupply/n1:Range1InputFrequencyHigh") & " Hz", "NULL Hz", "-")
			Range1InputFrequencyLow = Replace(ParseXML(xmlDom, "/n1:OMC_PowerSupply/n1:Range1InputFrequencyLow") & " Hz", "NULL Hz", "-")
			Range1InputVoltageHigh = ParseXML(xmlDom, "/n1:OMC_PowerSupply/n1:Range1InputVoltageHigh") 
			Range1InputVoltageHigh = Replace(ParseNumber( Range1InputVoltageHigh ) / 1000 & " V", "0 V", "-")
			Range1InputVoltageLow = ParseXML(xmlDom, "/n1:OMC_PowerSupply/n1:Range1InputVoltageLow") 
			Range1InputVoltageLow = Replace(ParseNumber( Range1InputVoltageLow ) / 1000 & " V", "0 V", "-")
			
			updateSummaryHealthCounts HealthState
			
			sXmlDocContent = sXmlDocContent & "<sensor type='OMC_PowerSupply' caption='" & Caption & "' total_output_power='" & TotalOutputPower & "' range_1_input_frequency_high='" & Range1InputFrequencyHigh & "' range_1_input_frequency_low='" & Range1InputFrequencyLow & "' range_1_input_voltage_high='" & Range1InputVoltageHigh & "' range_1_input_voltage_low='" & Range1InputVoltageLow & "' enabled_default='" & EnabledDefault & "' enabled_state='" & EnabledState & "' health_state='" & HealthState & "'/>"
	...
	Public Function ParseNumber( inoValue )
		If IsNumeric( inoValue) Then
			ParseNumber = CLng( inoValue )
		Else
			ParseNumber = 0
		End If
	End Function
...
' get psu information
sXmlDocContent = sXmlDocContent & "<psucheck>"
getPsuCheckInformation
sXmlDocContent = sXmlDocContent & "</psucheck>"
