...
If ($settingsContent['247CHECK']['ACTIVE'] -eq '1') {
	...
	If ($MSSQL) {
		...
		If ($SqlInstances.count -gt 0) {
            ...
            $LogAndDescription = 'Application|MSSQL Login Failed'
		    $oldChecks = Get-MAXfocusCheckList EventLogCheck log $LogAndDescription 
			If (!($oldChecks)) {
                $Flags = '32513'
                $Source = 'MSSQLSERVER'
                $Id = '18456'
				New-MAXfocusCheck EventLogCheck $NextUid $LogAndDescription $Flags $Source $Id
				$NextUid++
			}
